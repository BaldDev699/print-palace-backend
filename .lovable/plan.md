...
- `profiles` (+ auto-create trigger on signup) and `user_roles` (`app_role`: customer, manufacturer, admin) with a `has_role()` security-definer function. _(New — the original lacked a safe roles table; needed for manufacturer/admin gating.)_

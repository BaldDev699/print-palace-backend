...
const collectionItems = [
  {
    ...
    description:
      "Our classic tee, embodying minimalist style and comfort. Made with 100% organic cotton.",
    price: "$29.99",
  },
  {
    id: 2,
    ...
    description:
      "A premium hoodie featuring a unique handcrafted design. Perfect for a stylish, relaxed look.",
    price: "$59.99",
  },
  {
    id: 3,
      ...
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <section className="text-center mb-16">
          ...
          <p className="text-lg sm:text-xl text-muted-foreground max-w-3xl mx-auto">
            Explore exclusive designs by Roge. Each piece embodies a unique blend of artistry,
            quality, and modern aesthetics, crafted for the discerning individual.
          </p>
        </section>
        ...
        {featuredItem && (
          <section className="mb-16">
            <Card
              key={featuredItem.id}
              className="overflow-hidden shadow-2xl flex flex-col md:flex-row group rounded-lg"
            >
              ...
              <div className="md:w-2/5 lg:w-1/3 flex flex-col bg-card">
                {" "}
                {/* Ensure card background for text area */}
                <CardHeader className="pb-3 pt-6 px-6 md:pt-8">
                  <CardTitle className="text-3xl lg:text-4xl font-bold text-card-foreground">
                    {featuredItem.name}
                  </CardTitle>
                </CardHeader>
                ...
                <div className="p-6 pt-0">
                  <Button className="w-full text-lg py-3" size="lg" asChild>
                    <Link to={`/designer?template=${encodeURIComponent(featuredItem.name)}`}>
                      Customize & Buy
                    </Link>
                  </Button>
                </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
              {otherItems.map((item) => (
                <Card
                  key={item.id}
                  className="overflow-hidden flex flex-col group rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out"
                >
                  ...
                  <div className="p-6 pt-0">
                    <Button className="w-full" asChild>
                      <Link to={`/designer?template=${encodeURIComponent(item.name)}`}>
                        Customize & Buy
                      </Link>
                    </Button>
                  </div>

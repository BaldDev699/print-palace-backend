...
export const useCollageFrames = (fabricCanvas: FabricCanvas | null) => {
  ...
  const calculateFrameBounds = useCallback(
    (cell: any, canvasWidth: number, canvasHeight: number) => {
  ...
  const createCollage = useCallback(
    (layout: CollageLayout, mode: "apply" | "add" = "apply") => {
  ...
  const fillFrame = useCallback(
    (frameId: string, file: File | string) => {
  ...
  const swapFrameImages = useCallback(
    (frameId1: string, frameId2: string) => {
